...
var flowListObj = {
        ...
        $.ajax({
            ...
            success: function (resData) {
                ...
                if (state == 1) {
                    ...
                    if(resData.data.member && resData.data.member.length == 1){
                        if(resData.data.member[0].roleId){
                            var role = null;
                            try{
                                role = parseInt(resData.data.member[0].roleId);
                            }catch (e) {
                                // do nothing
                            }
                            if(role == 7) {
                    ...
                    console.log(resData.data.member)
                    ...
                    str_100 = arr_100
                    console.log(str_60)
                    console.log("speed=" + str_100)
                    //判断没有数据隐藏
                    if (productName) {
                        $(".productName").addClass('show');
                    } else {
                    ...
                    if (str_30) {
                        $(".teleNum").addClass('show');
                    } else {
                        $(".teleNum").addClass('hide');
                    }
                    if (str_40) {
                        $(".serialNumber").addClass('show');
                    } else {
                    ...
                    if (arr_60.length == 0) {
                        ...
                        $(".accNum").addClass('show');
                    }
                    if (arr_70.length == 0) {
                        $(".cloudBusinessNum").addClass('hide');
                    } else {
                        $(".cloudBusinessNum").addClass('show');
                    }
                    if (arr_80.length == 0) {
                        $(".userIp").addClass('hide');
                    } else {
                        $(".userIp").addClass('show');
                    }
                    if (arr_90.length == 0) {
                        $(".interIpAddr").addClass('hide');
                    } else {
                        $(".interIpAddr").addClass('show');
                    }
                    if (arr_100.length == 0) { //20190618 姬祥新增速率
                        $(".speed").addClass('hide');
                    } else {
                        $(".speed").addClass('show');
                    }
    ...
    , makeFlowArea: function (data) {
        var opt = {
            ...
            "imgPath": "../../../images/order/flow/", //图片路径
            "imgType": "png" //图片类型
        };
