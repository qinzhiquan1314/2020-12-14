...
var order_satisfaction = '', speed_satisfaction = '', attitude_satisfaction = '';
//鼠标点击保持当前状态
...
物流服务 extLevel3
总体评价  extLevel4
commentType 0：评价，1：建议， 2：光快线专线，3：宽带融合，4：移网*/
//获取路径中参数
...
$(document).ready(function () {
    ...
    var cloudDeliverMode = getUrlParam("cloudDeliverMode");

    //512   +     20    码销自提
    // 512   +     10     码销物流
    // 5121 +     10    行销物流
    // 5121 +     00    行销现场交付
    if (commentType == '7' && ((serviceType == '512' && cloudDeliverMode == '20') || (serviceType == '5121' && cloudDeliverMode == '00'))) {
        $('#onsiteComment').html('受理效率满意度')
    }
});
$.fn.serializeObject = function (para) {
    ...
    $(array).each(function () { // 遍历数组的每个元素 {name : xx , value : xxx}
        if (serializeObj[this.name]) {
            ...
            if (this.name == 'productStaff' || this.name == 'productHall' || this.name == 'productHotline' || this.name == 'serviceStaff' || this.name == 'serviceHall' || this.name == 'serviceHotline' || this.name == 'serviceHotline' || this.name == 'serviceHotline' || this.name == 'satisfactionNo') {
...
var orderAssessObj = {
    //变量-访问路径
    /*url: getOutUrl(getRootPath_web(),"/evaluation/submitEvaluation?flag=out&commentType="+commentType+"&orderNum="+orderNum)*/
    //获取数据   提价按钮
    assessSubmit: function () {
        ...
        if (code == "" && flag != 0) {
            ...
            if (commentType == 4 || commentType == 7) {
            ...
            $.ajax({
                ...
                success: function (resData) {
                    ...
                    if (resData.state == "1") {
                        ...
                        layer.msg('评价失败！' + resData.message, {
                            time: 2000 //2s后自动关闭
                        });
                    }
                }
    ...
    , checkData: function () {
        ...
        var v6 = $('input[name="extLevel5"]').val();
        ...
        /*else if (comment == "" || comment.indexOf("请输入") > -1) {
            layer.msg('请填写评价内容', {
                time: 2000 //2s后自动关闭
            });
            return false;
            ...
            return false;
        } else if ($('#productComment').attr('id')) {
