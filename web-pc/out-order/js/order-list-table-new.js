...
$(function() {
    ...
    $('#btn').click(function() {
        window.location.href = getOutUrl(getRootPath_web(), '/web-pc/out-order/page/cheak-the-order.html?&userName=' + userName + '&exCode=' + exCode + '&callCode=' + callCode);
    });
    $('#kd_Search').click(function() {
        window.location.href = "http://iservice.10010.com/e4/query/others/mobile_broadband-iframe.html?menuCode=000400200001";
    });

});

function initDataMe() {
    orderListObj.initTable(null, userName, resultCallback, '#table1', '');
    orderListObj.initTable(userName, '', resultCallback, '#table2', '');
    orderListObj.initTable(null, userName, resultCallback, '#table3', trouble);
}

//本页面对象
var orderListObj = {
    //初始table数据
    initTable: function(serialNumber, phoneNum, callback, div, trouble) {
            var data = {};
            var contentType = 'application/x-www-form-urlencoded; charset=UTF-8';
            var url;
            if (serialNumber && !trouble) {
                data.serialNumber = serialNumber
                url = getOutUrl(getRootPath_web(), '/trade/queryOrderOutNum?&userName=' + userName + '&exCode=' + exCode + '&callCode=' + callCode)

            } else if (phoneNum && !trouble) {
                url = getOutUrl(getRootPath_web(), '/trade/queryOrderOutPhone?&userName=' + userName + '&exCode=' + exCode + '&callCode=' + callCode)
            ...
            $(div).bootstrapTable({
                ...
                responseHandler: function(res) {
                    ...
                    $('#uls>li').eq($(div).parents('.container').attr('data-index')).attr('total', res.total)
                    $('#uls>li>span').eq($(div).parents('.container').attr('data-index')).html('(' + res.total + ')');
        ...
        //数据获取
        ,
    troubleActionFormatter: function(value, row, index) {
        if (row.status == "已办结") {
            if (row.markFlag == "1") {
...
window.actionEvents = {
    ...
    'click .showHistory': function(e, value, row, index) {
        ...
        if (row.from == "WYS") {
            htmlUrl = getOutUrl(getRootPath_web(), "/web-pc/out-order/page/order-assess-show-shiftNet-new.html?commentType=7&orderNum=" + orderNum + "&expNo=" + expNo + '&serviceType=' + row.serviceType + '&cloudDeliverMode=' + row.cloudDeliverMode);
        }
        /*if(netTypeCode == 41 || netTypeCode == 67){
            //光快线专线
