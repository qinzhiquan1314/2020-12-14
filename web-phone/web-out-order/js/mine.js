...
function initEvent() {
    ...
    $('#queryOrderButton').click(function () {
        var l = '/web-phone/web-out-order/page/check-the-order.html';
    ...
    $('#myLink').click(function () {
        window.location.href = getOutUrl(getRootPath_web(), "/web-phone/web-out-order/page/mine.html");
    });
...
function breakInit() {
    if (window.location.href.indexOf('/web-phone/web-out-order/page/check-the-order.html') > -1) {
        return true;
    }
}
...
function initData() {
    ...
    $('#serialNumberShow').html(userName);
    getData(null, userName, resultCallback, 'resultPhoneNumber');
    getData(userName, null, resultCallback, 'resultSerialNumber');
    getData(null, userName, troubleResultCallback, 'resultTrouble', 'trouble');
...
function getUrl(url, userName, exCode, callCode) {
    if (url.indexOf('?') > -1) {
        url = url + '&userName=' + userName + '&exCode=' + exCode + '&callCode=' + callCode + '&userName2=' + userName2;
    } else {
        url = url + '?userName=' + userName + '&exCode=' + exCode + '&callCode=' + callCode + '&userName2=' + userName2;
    }
...
function getData(serialNumber, phoneNum, callback, div, trouble) {
    var data = {};
    var contentType = 'application/x-www-form-urlencoded; charset=UTF-8';
    var url;
    if (serialNumber && !trouble) {
        data.serialNumber = serialNumber
        url = getUrl(getRootPath_web() + "/trade/queryOrderOutNum", userName, exCode, callCode)
    } else if (phoneNum && !trouble) {
        url = getUrl(getRootPath_web() + "/trade/queryOrderOutPhone", userName, exCode, callCode)
        data.phoneNumber = phoneNum;
    } else if (trouble) {
        url = getUrl("https://wxzc.bjunicom.com.cn/queryCenter/web-phone/web-out-order/page/balkIndex2", userName, exCode, callCode)
        data.smcode = '885249';
        data.phoneNumber = phoneNum
    ...
    $.ajax({
        ...
        url: url,
        success: function (r) {
            callback(r, serialNumber, phoneNum, div, trouble);
        },
        error: function (data) {
            alert('响应失败！');
        }
...
function troubleResultCallback(r, serialNumber, phoneNum, div) {
    var that = this;
    if (r && r.respCode == '200') {
        ...
        for (var i = 0; i < r.data.length; i++) {
            ...
            $('#' + div).find('[newResult=true]').each(function () {
                ...
                if (item.markFlag == '0') {
                    ...
                    clickLabel = "查看评价";
                    clickBehavior = getOutUrl(getRootPath_web(), "/web-phone/web-out-order/page/order-zqza-assess-new-show.html?qryType=balkNo&qryNumber=" + item.balkNo);
                }
...
function resultCallback(r, serialNumber, phoneNum, div) {
    var that = this;
    if (r && r.state == '1') {
        if (serialNumber) {
            ...
            for (var i = 0; i < r.rows.length; i++) {
                ...
                $('#' + div).find('[newResult=true]').each(function () {
                    ...
                    if (item.statusFlag == "订单完成" || item.statusFlag == "已完成") {
                        if (commentState == "1" || commentState == "2" || commentState == "3" || commentState == "4" || commentState == "5" || commentState == "6") {
                    ...
                    if (clickLabel) {
                        _this.find('.hh6 > .de-hh6 > .btn1').html(clickLabel);
                        _this.find('.hh6 > .de-hh6 > .btn1').click(function () {
                                window.location.href = clickBehavior + "&flag=" + '0' + "&phoneNum=" + (serialNumber ? serialNumber : phoneNum);
                    ...
                    _this.find('.hh6 > .de-hh6 > .btn2').click(function () {
                            window.location.href = href.href1 + "&flag=" + '0' + "&phoneNum=" + (serialNumber ? serialNumber : phoneNum);
                        }
                    );
                    _this.attr('newResult', false);
                    _this.show();
