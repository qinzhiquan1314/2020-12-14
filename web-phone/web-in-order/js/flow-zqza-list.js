...
var flowListObj = {
    ...
    getDate: function () {
        $.ajax({
            ...
            url: "https://wxzc.bjunicom.com.cn/queryCenter/web-phone/web-in-order/page/balkFlowList?userName="+userName+"&exCode="+exCode+"&callCode="+callCode,
...
function getRadio(){
    ...
    $.ajax({
        ...
        url:"https://wxzc.bjunicom.com.cn/queryCenter/web-phone/web-in-order/page/ifmGovUserUrge?userName="+userName+"&exCode="+exCode+"&callCode="+callCode,
        contentType: "application/json;charset=UTF-8",
        data:JSON.stringify({
            "balkno": balkNo ,
            "remark" : remark,
        ...
        success : function(resData) {
            ...
            var href=getOutUrl(getRootPath_web(),"/web-phone/web-in-order/page/flow-zqza-list.html?balkNo="+balkNo+"&secCode="+secCode);//故障处理详情
            console.log(desc);
            if(state == 200) {
                layer.open({
                    content: '催修成功'
...
function tzmessage(){
    if (message == 1) {
        var href=getOutUrl(getRootPath_web(),"/web-phone/web-in-order/page/order-zqza-search-validate.html");
        window.location.href = href;
    }else{
        goBack();
    }
