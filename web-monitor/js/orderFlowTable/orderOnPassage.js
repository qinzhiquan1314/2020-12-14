//报表标识  默认属地（OrderOnProcessByArea）,发展（OrderOnProcessByDevelop）
var reportKey="OrderOnProcessByArea";
//校验标识
...
$(function () {
		if (fileName ==null||fileName =="") {
			...
			//分类口径和销售线回显
			if (reportKey=="OrderOnProcessByArea") {  
...
$("#branchInfo ul li").click(function (){
     ...
     branchFlag=-1;
     if (clickFlag==0) {
    	 reportKey="OrderOnProcessByArea";
	}else{
...
$("#searchBtn").click(function(){
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="OrderOnProcessByArea"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	}else if(reportKey=="OrderOnProcessByDevelop"&&$('#selectBranch').find("option:selected").val()!=""
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey=="OrderOnProcessByArea"&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val()
		}else if(reportKey=="OrderOnProcessByDevelop"&&$('.developItem').eq(branchFlag).find("option:selected").val()
			){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "OrderOnProcessByArea,OrderOnProcessByDevelop";
