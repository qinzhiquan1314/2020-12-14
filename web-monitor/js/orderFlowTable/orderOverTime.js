//报表标识  默认属地（overCountMonitor）,发展（overCountMonitorDev）
var reportKey="overCountMonitor";
//校验标识
...
$("#searchBtn").click(function(){
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="overCountMonitor"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	} else if(reportKey=="overCountMonitorDev"&&$('#selectBranch').find("option:selected").val()!=""
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey=="overCountMonitor"&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val()
		}else if(reportKey=="overCountMonitorDev"&&$('.developItem').eq(branchFlag).find("option:selected").val()){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "overCountMonitor,overCountMonitorDev";
