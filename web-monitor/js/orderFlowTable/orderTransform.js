//报表标识  默认属地（OrderConvertByArea）,发展（OrderConvertByDevelop）
var reportKey="OrderConvertByArea";
//校验标识
...
$(function () {
		if (fileName ==null||fileName =="") {
			...
			//分类口径和销售线回显
			if (reportKey=="OrderConvertByArea") {  
...
$("#searchBtn").click(function(){
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="OrderConvertByArea"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	}else if(reportKey=="OrderConvertByDevelop"&&$('#selectBranch').find("option:selected").val()!=""
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey=="OrderConvertByArea"&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val();
		}else if(reportKey=="OrderConvertByDevelop"&&$('.developItem').eq(branchFlag).find("option:selected").val()){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "OrderConvertByArea,OrderConvertByDevelop";
