//报表标识  默认属地（OrderBackByArea）,发展（OrderBackByDevelop）
var reportKey="OrderBackByArea";
//校验标识
...
$("#searchBtn").click(function(){
	...
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="OrderBackByArea"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	}else if(reportKey=="OrderBackByDevelop"&&$('#selectBranch').find("option:selected").val()!=""
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey=="OrderBackByArea"&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val()
		}else if(reportKey=="OrderBackByDevelop"&&$('.developItem').eq(branchFlag).find("option:selected").val()){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "OrderBackByArea,OrderBackByDevelop";
