...
var reportName="orderDetailNewByArea";
//校验标识
...
$("#searchBtn").click(function(){
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey==reportKeyByArea&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val()
		}else if(reportKey==reportKeyByDevelop&&$('.developItem').eq(branchFlag).find("option:selected").val()){
