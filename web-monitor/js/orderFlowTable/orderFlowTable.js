//报表标识  默认属地（flowMonitorTotal2）,发展（flowMonitorTotalDev2）
var reportKey="flowMonitorTotal2";
//校验标识
...
$(function () {
		if (fileName ==null||fileName =="") {
			...
			//分类口径和销售线回显
			if (reportKey=="flowMonitorTotal2") {  
...
$("#searchBtn").click(function(){
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="flowMonitorTotal2"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	}else if(reportKey=="flowMonitorTotalDev2"&&$('#selectBranch').find("option:selected").val()!=""
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		}else if(reportKey=="flowMonitorTotalDev2"&&$('.developItem').eq(branchFlag).find("option:selected").val()){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "flowMonitorTotal2,flowMonitorTotalDev2";
