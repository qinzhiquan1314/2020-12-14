//报表标识  默认属地（flowMonitorTotal1）,发展（flowMonitorTotalDev1）
var reportKey="flowMonitorTotal1";
//校验标识
...
$(function () {
		if (fileName ==null||fileName =="") {
			...
			//分类口径和销售线回显
			if (reportKey=="flowMonitorTotal1") {  
...
$("#branchInfo ul li").click(function (){
     ...
     branchFlag=-1;
     if (clickFlag==0) {
    	 reportKey="flowMonitorTotal1";
	}else{
		 reportKey="flowMonitorTotalDev1";
	}
...
$("#searchBtn").click(function(){
	...
	if(branchFlag==-1 && $("#selectBranch").prop("disabled")){
	...
	}else if(reportKey=="flowMonitorTotal1"&&$('#selectBranch').find("option:selected").val()!=""
		...
		return;
	}else if(reportKey=="flowMonitorTotalDev1"&&$('#selectBranch').find("option:selected").val()!=""
		&&$('.developItem').eq(branchFlag).find("option:selected").val()==""){
		console.log($('#branchItem').val());
		var selectBranch = $('#selectBranch').find("option:selected").val();
		if (selectBranch.indexOf("中心")!=-1) {
			layer.msg('请选择部门中心！', {
	 			time: 2000 //2s后自动关闭
	...
	if (branchFlag==-1 && !$("#selectBranch").prop("disabled")) {
		...
		if (reportKey=="flowMonitorTotal1"
			&&$('.branchItem').eq(branchFlag).find("option:selected").val()) {  //获取部门信息
			areaCode = $('.branchItem').eq(branchFlag).find("option:selected").val()
		}else if(reportKey=="flowMonitorTotalDev1"
			&&$('.developItem').eq(branchFlag).find("option:selected").val()){
...
$("#historySearch").click(function(){
	...
	reportKeyHis = "flowMonitorTotal1,flowMonitorTotalDev1";
